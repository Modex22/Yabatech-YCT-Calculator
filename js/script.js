...
// get bonus score for 1/2 sittnig
  const bonusScore = () => parseInt(document.querySelector('#bonus-input').value, 10);
  ...
  // get total aggregate
  const totalAgg = (utme, pUtme, waec, bonus) => parseFloat(utme + pUtme + waec + bonus).toFixed(2);
  ...
  const showScore = () => {
    ...
    const score = totalAgg(utme, pUtme, waec ,bonus);
    let i = 0.00;
